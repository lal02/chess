...
public class CheckTest {
    //Knight checks done
    //diagonal and straight checks done
    //queen checks -> not neccessary because queen uses rook and bishop mechanics also in the gamelogic
    //pawn checks done
    //en passant checks done
    //check prevention -> moves that stop a check should correctly stop the check in the move validation -> backwards bishop move bug in whiterooktest
    //check prevention:
    /*
    pawn moves
    block check with bishop, rook,knight
    -> also backwards moves
    stop checks by capturing the piece, knight, stop checks by playing pawn 2 forward
     */
